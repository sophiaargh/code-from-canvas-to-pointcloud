    ...
    def __init__(self, style: str):
        ...
        for scene in scenes:
            ...
            for img_name in sorted(os.listdir(real_scene_path)):
                # Break early if we already hit the 50-photo limit for this scene
                if scene_photo_count >= 5:
                    break
...
def train_adapter():
    ...
    dataloader = DataLoader(
        ...
        shuffle=True, 
        collate_fn=mapanything_collate_fn  # <-- Pass our custom collate here
    )
...
# 3. Extract the parameters and manually recreate them
    ...
    for epoch in range(args.epochs):
        epoch_loss = 0.0
        for batch_real, batch_style in dataloader:
            ...
            # EVERYTHING BELOW THIS LINE MUST BE UN-INDENTED
            if isinstance(batch_style, list) and isinstance(batch_style[0], dict):
                batch_style_cuda = [{k: v.to(device) if isinstance(v, torch.Tensor) else v 
                                     for k, v in view.items()} for view in batch_style]
            elif isinstance(batch_style, dict):
    ...
    # Save learned parameters for inference evaluation
    folder_path = './eval_pipeline/weights'
    save_path = f"{folder_path}/adapter_{args.style}_{args.norm_num_blocks}_{'end' if  args.norm_from_end else 'front'}.pth"
    ...
    timed_print(f"Saved weights to {save_path}")
